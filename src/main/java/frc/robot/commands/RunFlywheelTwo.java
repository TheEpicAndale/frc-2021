...
public class RunFlywheelTwo extends CommandBase {
    public static double secondEndVoltage;
    /** Creates a new RunFlywheelTwo. */
    ...
    public void execute() {
        ...
        if (Shooter.flywheel.shootIteration == 1) {
            ...
            voltage = RunFlywheel.endVoltage + 0.02 * (Timer.getFPGATimestamp() - initTime);
        } else {
            System.out.println("Shoot Iteration is wrong!");
            quitRamp = true;
        }

        secondEndVoltage = voltage;
        Shooter.flywheel.setVolt(voltage);

        System.out.println("VOLTAGE 2: " + voltage);
        System.out.println("RPM 2: " + Shooter.flywheel.getRPM());
        System.out.println("DESIRED RPM 2: " + desiredRPM);
    }
